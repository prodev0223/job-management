...
import { useGetUserByIdQuery, useGetUsersFromXeroQuery, useUpdateUserDetailsMutation } from '../../../redux/slices/userApiSlice';
...
const UserEdit = () => {
    ...
    const [updateUser, { isLoading: isLoadingUpdateUser, isSuccess: isSuccessUpdateUser, isError: isErrorUpdateUser, error: errorUpdateUser }] = useUpdateUserDetailsMutation();
    ...
    const formik = useFormik({
        initialValues: {
            employeeId: user?.employeeId ? user.employeeId : 0o0,
            roleId: user?.roleId ? user.roleId._id : '',
            firstName: user?.firstName ? user.firstName : '',
            lastName: user?.lastName ? user.lastName : '',
            email: user?.email ? user.email : '',
            xero: {
                employeeId: user?.xero?.employeeId ? user.xero.employeeId : '',
                currentTimesheetId: user?.xero?.currentTimesheetId ? user.xero.currentTimesheetId : '',
                previousTimesheetId: user?.xero?.previousTimesheetId ? user.xero.previousTimesheetId : '',
                earningsRateId: user?.xero?.earningsRateId ? user.xero.earningsRateId : '',
            },
    ...
    if (isLoadingUser || isLoadingXeroUsers || isLoadingRoles || isLoadingUpdateUser) {
        content = (
            <React.Fragment>
    ...
    } else if (isErrorUser || isErrorXeroUsers || isErrorRoles || isErrorUpdateUser) {
        content = (
            <React.Fragment>
                <Container fluid className="p-0">
    ...
    } else if (isSuccessUser && isSuccessXeroUsers && isSuccessRoles && roles && user && xeroUsers) {
        ...
        content = (
            <React.Fragment>
                <Helmet title={`${user.firstName} ${user.lastName} - Details`} />
                <Container fluid className="p-0">
                    <Row>
                        <Col>
                            <Card>
                                    ...
                                    <Col xs="auto" className="ms-auto text-end">
                                        <Card.Header>
                                            <Row>
                                                ...
                                                <Col>
                                                    <Button variant="secondary" onClick={() => navigate(`/users/${userId}`)}><ArrowLeft size={18} /></Button>
                                                </Col>
                                            </Row>
                                        </Card.Header>
                                    </Col>
                                ...
                                <Card.Body>
                                    <Form onSubmit={formik.handleSubmit}>
                                        ...
                                        {isErrorUpdateUser && (
                                            <Alert className="my-3" variant="danger">
                                                <div className="alert-message">{JSON.stringify(errorUpdateUser, null, 2)}</div>
                                            </Alert>
                                        )}
                                        <Row className="mb-3">
                                            ...
                                            <Form.Group className="mb-3" as={Col} md={2}>
                                                <Form.Label><strong>Assigned Role</strong></Form.Label>
                                                <CustomSelect
                                            ...
                                            <Form.Group className="mb-3" as={Col} md={4}>
                                                <Form.Label>
                                                    <strong>Mapped Xero Employee</strong>
                                                </Form.Label>
                                                <CustomSelect
                                                    className="react-select-container"
                                                    //@ts-ignore
                                                    onChange={value => formik.setFieldValue('xero.employeeId', value.value)}
                                        ...
                                        <Row>
                                            ...
                                            <Form.Group className="mb-3" as={Col} md={2}>
                                                ...
                                                <Form.Control
                                                    ...
                                                    onChange={formik.handleChange}
                                                    size="lg"
                                                    placeholder="Enter pincode to change"
