...
const JobCreate = () => {
    ...
    const formik = useFormik({
        initialValues: {
            ...
            description: '',
            dateSubmitted: dayjs(todayDate).format('DD/MM/YYYY'),
            followUpDate: '',
            reqDateOfQuote: '',
            doAndCharge: false,
            estimator: '',
        ...
        onSubmit: async (values, { setErrors, setStatus, setSubmitting }) => {
            try {
                ...
                if (clientTeamCreated) {
                    // alert(JSON.stringify(clientTeamCreated.id))
                    let name = clients && clients.find(client => client._id === formik.values.client._id)?.name;
                    if (name) {
                        updateClient({
                            clientId: formik.values.client._id,
                            client: {
                                name: name,
                                isActive: true,
                ...
                let year = values.dateSubmitted.split('/')[2]
                let month = values.dateSubmitted.split('/')[1]
                let day = values.dateSubmitted.split('/')[0]
                console.log('dateSubmitted', `${year}-${month}-${day}`)
    ...
    const handleCreateClientTeams = (clientId: any) => {

        console.log('clientId', clientId);
        if (clients) {
            let client = clients && clients.find(client => client._id === clientId)

            console.log('client', client);

            if (client) {
                createTeamsChannel(client.name);
    ...
    if (isSuccessTeamsChannelCreate) {
        if (clientTeamCreated) {
            // alert(JSON.stringify(clientTeamCreated.id))
            let name = clients && clients.find(client => client._id === formik.values.client._id)?.name;
            if (name) {
    ...
    if (isLoadingStatuses || isLoadingClients || isLoadingUsers) {
        content = (
            <React.Fragment>
    ...
    } else if (isSuccessStatuses && isSuccessClients && isSuccessUsers) {
                                                ...
                                                <Card.Body>
                                                    <Row>
                                                        ...
                                                        <Form.Group className="mb-3" as={Col} md={2}>
                                                            ...
                                                            <ClientCreate />
                                                            {/* <Link style={{ textDecoration: 'none' }} to={`/clients/create`} target="_blank" rel="noopener noreferrer">
                                                    ...
                                                    <Row>
                                                        <Form.Group>
                                                            {formik.values.client._id !== '' || undefined ? (
                                                                <Form.Label>
                                                                    {clients && clients.find(client => client._id === formik.values.client._id)?.teamsId === 'UNKNOWN' ? (
                                                                        <>
                                                                            {isLoadingTeamsChannelCreate ? (
                                                                                <Loader />
                                                                            ) : !isSuccessTeamsChannelCreate ? (
                                                                                <Button onClick={() => navigate(`/clients/${formik.values.client._id}`)}>
                                                                                    Create Teams
                                                                                </Button>
                                                                            ) : null}
                                                                            ...
                                                                            {isSuccessTeamsChannelCreate ? (
                                                                                <Form.Label>
                                                                                    Teams Channel Created! {console.log('clientTeamCreated', clientTeamCreated)} <CheckCircle size={14} color="green" />
                                                                                </Form.Label>
                                                                            ) : isErrorTeamsChannelCreate ? (
                                                                                <ErrorAlert title="Error creating team!" error={JSON.stringify(errorTeamsChannelCreate, null, 2)} />
                                                                            ) : null}
                                                                        </>
                                                                    ) : (
                                                                        <Form.Label>
                                                                            <CheckCircle size={14} color="green" /> {"  "} Teams ID Found
                                                                        </Form.Label>
                                                                    )}
                                                                </Form.Label>
                                                            ) : null}
    ...
    } else if (isErrorStatuses || isErrorClients || isErrorUsers || isErrorJobCreate) {
        content = (
            <React.Fragment>
                <Container fluid className="p-0">
                    <Row>
